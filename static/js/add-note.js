    ...
    accept: function (file, done) {
        if (file.type === 'application/pdf') {
            if (this.files.filter(f => f.type === 'application/pdf').length > 1) {
                ...
                $(this['element']).closest('.card-body').find('.embed-responsive-item').attr('src', fileUrl)
            }
        } else {
            done();
        }
...
function addSection(skipScroll = false) {
    ...
    newSection.find('.close').on('click', function (evt) {
        ...
        currentForm.remove()
        window.scrollTo({
            top: prevForm.get(0).getBoundingClientRect().top + window.pageYOffset - 25,
            behavior: 'smooth',
        })
    });
    ...
    if (skipScroll) {
        window.scrollTo({
            top: newSection.get(1).getBoundingClientRect().top + window.pageYOffset - 25,
            behavior: 'smooth',
        })
    }
}
...
$(document).ready(function () {
    ...
    $('#submit-note').on('click', function (evt) {
        ...
        forms.each(function () {
            ...
            dropzone.files.forEach(file => allSectionFormData.append(`s${counter}_files`, file))
            counter++;
        });
        $.ajax({
            type: 'POST',
